...
These chains are the building blocks of more complex chains for processing unstructured text data and receive both documents and a question as input. They then utilize the language model to provide an answer to the question based on the given documents.

- `StuffDocumentsChain`: This chain is the simplest of the 3 chains and simply injects all documents into the prompt. It then returns the answer to the question, using all documents as context. It is suitable for QA tasks over a small number of documents.
